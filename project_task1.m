...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  %  We were asked to "Extend the MATLAB code from your LAB ASSIGNMENT
  %  to incorporate four farmers." So, this is an expansion of the task 3
  %  of the lab assignment, and not just an expansion of the provided sample
  %  code. Therefore, this is not delivering water after a fixed time,
  %  but is rather using a controller to determine when water is needed. 
  %  Also, I_total = 3 here to accomodate four farms. 

  
  % OPTION 2: following is the controller that determines whether each of the fields
  % needs irrigation or not
  
    irrigated = false;

    if ((M1(t) < Moist_min) && (I(t) > 0))
...
%%    
    ...
    pause(0.5);   % stalls the code before processing next t. Creates a video-like effect on figures
end
